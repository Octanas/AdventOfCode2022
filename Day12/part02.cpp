...
int main()
{
    ...
    int LowestTentativeDistancePath = INT_MAX;

    // Run the Dijkstra algorithm for each starting node of height 1
    for (int NodeIndex = 0; NodeIndex < Nodes.size(); ++NodeIndex)
    {
        if (Nodes[NodeIndex]->Height == 1)
        {
            // Reset tentative distance of all nodes
            for (int i = 0; i < Nodes.size(); i++)
            {
                Nodes[i]->TentativeDistance = INT_MAX;
            }
            ...
            // Keep the lowest tentative distance
            if (EndNode->TentativeDistance < LowestTentativeDistancePath)
            {
                LowestTentativeDistancePath = EndNode->TentativeDistance;
            }
        }
    }

    cout << LowestTentativeDistancePath << endl;
}