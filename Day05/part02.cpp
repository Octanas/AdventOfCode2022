...
int main()
{
    ...
    while (getline(MyReadFile, StrCrates))
    {
        if (ParsedCrates)
        {
            ...
            // Crates get placed in the same order.
            // Put them in a temporary stack between removing them and placing them
            // so that they are placed in the same order
            stack<char> MovementStack;
            ...
            for (int i = 0; i < NumCrateToMove; ++i)
            {
                ...
                MovementStack.push(Crate);
            }

            // Place crates in new stack
            for (int i = 0; i < NumCrateToMove; ++i)
            {
                const char Crate = MovementStack.top();
                MovementStack.pop();
